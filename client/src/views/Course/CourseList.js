...
class UniversityList extends Component {
  ...
  editData(item){
    ...
    this.props.history.push({
      pathname: '/main/course/edit/'+item._id,
      query: item
   })
  }
  ...
  DeleteQuery = (id) => {
      ...
      fetch("/api/course/"+id,{method:'DELETE',body:null})
      .then(res => res.json())
      .then((result) => {
...
addRow = () =>{
  ...
  handleSearchInput(e){
    ...
    const list = CourseFilter(universitiesList,e.target.value);
...
const floatingActionButtonProps =
    ...
    const columns = [ {
      ...
      filterIcon: filtered => <Icon type="smile-o" style={{ color: filtered ? '#108ee9' : '#aaa' }} />,
      onFilter: (value, record) => record.coursecode.toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: (visible) => {
        if (visible) {
          setTimeout(() => {
            this.searchInput.focus();
    ...
    // {
    //   title: 'Colege',
    //   dataIndex: 'college_id.name',
    //   key: 'name',
    //   filters: this.state.filter_uni,
    //   onFilter: (value, record) => record.college_id.name.indexOf(value) === 0,
    // }
  ];

    if(this.state.universitiesList != null){
      university  = this.state.universitiesList.map(university => (
       <CourseItem key ={university._id} university = {university}/>
      ));
    }
    else{
